...
for vehicle_count in range(1,vehicle_count+1):
    ...
    #Tout camion commence et finit sa tournée par VDR
    for k in range(vehicle_count):
        problem += pulp.lpSum(x[0][j][k] for j in range(1,customer_count)) == 1
        problem += pulp.lpSum(x[i][0][k] for i in range(1,customer_count)) == 1

    #Tout camion entrant dans une PFC doit en ressortir
    for k in range(vehicle_count):
        for j in range(customer_count):
            problem += pulp.lpSum(x[i][j][k] if i != j else 0 
    ...
    for k in range(vehicle_count):
        problem += pulp.lpSum(df.nb_colis[j] * x[i][j][k] if i != j else 0 for i in range(customer_count) for j in range (1,customer_count)) <= vehicle_capacity 
...
color_list=['red','blue','yellow','pink','black','orange']

for k in range(vehicle_count):
    for i in range(customer_count):
        for j in range(customer_count):
            if i != j and pulp.value(x[i][j][k]) == 1:
                plt.plot([df.longitude[i], df.longitude[j]], [df.latitude[i], df.latitude[j]], c=color_list[k//3])
                if i==0:
